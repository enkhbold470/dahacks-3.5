          ...
          {hackathonData.timeline.map((item, index) => (
            ...
            >
              ...
              <div className={`w-5/12 ${index % 2 === 0 ? 'pr-8 text-right' : 'pl-8 text-left'}`}>
                <div className={`p-4 rounded-lg transition-all duration-300 ${activeIndex === index ? 'bg-cyber-light/50 border border-neon-green/30' : 'bg-transparent'}`}>
                  <span className="font-mono font-bold text-neon-green">{item.time}</span>
                  <h3 className="text-lg font-cyber font-semibold text-white mt-1">{item.event}</h3>
                </div>
              </div>
