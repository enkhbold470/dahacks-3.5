...
const Sponsors = () => {
  ...
  const tierConfig = {
    Platinum: { gridCols: 1, size: 'h-32', color: 'neon-text-blue' },
  ...
  const PlaceholderLogo = ({ name }: { name: string }) => (
    <div className="flex items-center justify-center border-2 border-dashed border-gray-400 bg-cyber-light/20 rounded-md p-4 h-full">
      <span className="text-xl text-center font-cyber text-gray-300">{name}</span>
    </div>
  );
        ...
        {Object.entries(tierConfig).map(([tier, config]) => (
          sponsorsByTier[tier] && (
            <div key={tier} className="mb-16">
              <h3 className={`text-2xl font-cyber font-bold mb-6 text-center ${config.color}`}>
                {tier} Sponsors
              </h3>
              
              <div className={`grid grid-cols-1 md:grid-cols-${config.gridCols} gap-8`}>
